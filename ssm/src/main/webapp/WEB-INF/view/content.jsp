...
<body class="gray-bg">
        ...
        <div class="row">
			  ...
			  <div class="col-lg-2">
				<div class="input-group">
					...
					<select class="form-control" name="styleid" id = "styleid1">
						<option value="0">---请选择---</option>
						<c:forEach items="${styleList }" var="r">
						 	<option value="${r.id }">${r.sname }</option>
						</c:forEach>
                	</select>
				</div>
			 </div>
					...
					<form id="form_user" method="post" action="reserveContent.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>广告分类：</td>
								<td>
									<select class="form-control" name="styleid" id = "styleid" aria-required="true" required>
	...
	<div class="modal fade" id="modal_user_in" role="dialog" aria-labelledby="modal_user_edit" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user_in" method="post" action="in.htm" enctype="multipart/form-data">
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>导入excel：</td>
	...
	$(function () {
	    ...
	    $("#submit_form_user_btn").click(function(){
	        var result =false;
	        $.post("jiaoyan.htm",{"title":$("#title").val()},function (data) {
				if(data==""){
				    result=true;
	        ...
	        if($("#price").val()>1000||$("#price").val()<100){
	            alert("费用不能低于100或者大于1000");
	            result=false;
			}else {
	            result=true;
			}
	...
	var TableInit = function () {
	    ...
	    oTableInit.Init = function () {
	        $('#table_user').bootstrapTable({
	            ...
	            pageSize: 3,                       //每页的记录行数（*）
	            pageList: [3, 5, 50, 75, 100],    //可供选择的每页的行数（*）
