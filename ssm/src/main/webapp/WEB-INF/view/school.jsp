...
<body class="gray-bg">
        ...
        <div class="row">
			  <div class="col-lg-2">
				<div class="input-group">
			      <span class="input-group-addon">校名 </span>
			      <input type="text" name="aname" class="form-control" id="txt_search_username" >
				</div>
			  </div>
				...
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
								...
								<td>
									<select class="form-control" name="areaid" id = "areaid" aria-required="true" required>
										...
										<c:forEach items="${areasList}" var="r">
										 	<option value="${r.id}">${r.aname}</option>
										</c:forEach>
				                	</select>
								</td>
								<td>详细地址：</td>
							...
							<tr>
								<td>是否运营：</td>
								<td><input type="radio" id="statu1" name="status" value="运营">是
									<input type="radio" id="statu2" name="status" value="倒闭">否</td>
								<td>建校时间：</td>
    ...
    var TableInit = function () {
        ...
        oTableInit.Init = function () {
            $('#table_user').bootstrapTable({
                ...
                pageSize: 2,                       //每页的记录行数（*）
                pageList: [2, 25, 50, 75, 100],    //可供选择的每页的行数（*）
	...
	function initEditUser(getSelection){
        getSelection.status=='运营'? $('#statu1').prop("checked",true):$('#statu2').prop("checked",true);
//日期回显
