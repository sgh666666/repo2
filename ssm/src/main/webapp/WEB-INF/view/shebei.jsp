...
<link href="${path }/resources/css/style.css?v=4.1.0" rel="stylesheet">
	<script src="${path }/resources/js/echarts.js" rel="stylesheet"></script>
...
<body class="gray-bg">
				...
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveShebei.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
							<td>内存：</td>
							<td><input type="text" id="sbram" name="sbram"
									   class="form-control" aria-required="true" required/></td>
							<td>机身颜色：</td>
							...
							<tr>
								<td>设备类型：</td>
								<td>
									<select class="form-control" name="sbtypeid" id = "sbtypeid" aria-required="true" required>
										<c:forEach items="${sbtypeList }" var="r">
	...
	<%--柱状图--%>
	<div class="modal fade" id="modal_user_bar" role="dialog" aria-labelledby="modal_user_del" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_bar").click(function () {
                $('#modal_user_bar').modal({backdrop: 'static', keyboard: false});
                $("#modal_user_bar").show();
                $.post("bar.htm",{},function (data) {
                    var sbtype = new Array();
                    var num = new Array();
					var arr = JSON.parse(data);
                    for (var i = 0; i <arr.length ; i++) {
						sbtype.push(arr[i].tname);
						num.push(arr[i].count);
                    }
